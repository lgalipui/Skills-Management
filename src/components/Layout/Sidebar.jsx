...
export const Sidebar = () => {
  ...
  return (
    <aside className="w-64 bg-[#005021] text-white min-h-screen flex flex-col shadow-2xl relative z-20">
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        <p className="px-3 text-[10px] font-bold text-emerald-200/40 uppercase tracking-wider mb-2">Navegación</p>
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) => clsx(
                ...
                isActive 
                  ? "bg-white/10 text-white font-semibold shadow-[inset_3px_0_0_0_rgba(255,255,255,1)]" 
      ...
      <div className="p-3 bg-black/10 mt-auto border-t border-white/10">
        <div className="flex items-center gap-2.5 px-1 py-0.5">
          <img src={currentUser.avatar} alt="Avatar" className="w-8 h-8 rounded-full border border-emerald-400 shadow-md object-cover shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold text-white truncate leading-tight">{currentUser.name}</p>
            <p className="text-[10px] text-emerald-200/80 truncate mt-0.5 leading-none">{currentUser.role}</p>
            {orgName && (
              <p className="text-[9px] text-emerald-100/50 truncate mt-1 leading-none" title={orgName}>
                {orgName}
              </p>
            )}
          </div>
